        ...
        sql_insert = """
            ...
            gpa_tich_luy AS (
                SELECT
                    ...
                    bg.ma_sinh_vien,
                    ROUND(CASE
                        WHEN SUM(CASE WHEN bg.best_diem_he_4 IS NOT NULL THEN bg.so_tin_chi ELSE 0 END) > 0
                        THEN SUM(CASE WHEN bg.best_diem_he_4 IS NOT NULL THEN bg.best_diem_he_4 * bg.so_tin_chi ELSE 0 END)
                             / SUM(CASE WHEN bg.best_diem_he_4 IS NOT NULL THEN bg.so_tin_chi ELSE 0 END)
                        ELSE NULL
                    END, 2) AS gpa_he_4,
                    ROUND(CASE
                        WHEN SUM(CASE WHEN bg.best_diem_he_10 IS NOT NULL THEN bg.so_tin_chi ELSE 0 END) > 0
                        THEN SUM(CASE WHEN bg.best_diem_he_10 IS NOT NULL THEN bg.best_diem_he_10 * bg.so_tin_chi ELSE 0 END)
                             / SUM(CASE WHEN bg.best_diem_he_10 IS NOT NULL THEN bg.so_tin_chi ELSE 0 END)
                        ELSE NULL
                    END, 2) AS gpa_he_10,
                    SUM(bg.so_tin_chi) AS tong_tin_chi_dang_ky,
                    SUM(CASE WHEN bg.dat_mon = 1 AND bg.best_diem_he_4 IS NOT NULL THEN bg.so_tin_chi ELSE 0 END) AS tin_chi_dat,
                    SUM(CASE WHEN bg.dat_mon = 0 AND bg.best_diem_he_4 IS NOT NULL THEN bg.so_tin_chi ELSE 0 END) AS tin_chi_khong_dat,
                    COUNT(*) AS tong_mon_dang_ky,
                    SUM(CASE WHEN bg.dat_mon = 1 AND bg.best_diem_he_4 IS NOT NULL THEN 1 ELSE 0 END) AS so_mon_dat,
                    SUM(CASE WHEN bg.dat_mon = 0 AND bg.best_diem_he_4 IS NOT NULL THEN 1 ELSE 0 END) AS so_mon_khong_dat,
                    SUM(CASE WHEN bg.hoc_lai THEN 1 ELSE 0 END) AS so_mon_hoc_lai,
                    ROUND(CASE
                        WHEN SUM(CASE WHEN bg.best_diem_he_4 IS NOT NULL THEN 1 ELSE 0 END) > 0
                        THEN 100.0 * SUM(CASE WHEN bg.dat_mon = 1 AND bg.best_diem_he_4 IS NOT NULL THEN 1 ELSE 0 END)
                             / SUM(CASE WHEN bg.best_diem_he_4 IS NOT NULL THEN 1 ELSE 0 END)
                        ELSE NULL
                    END, 2) AS ty_le_dat
                FROM best_grade bg
            ...
            rl_avg AS (
                SELECT sinh_vien_key, ROUND(AVG(diem_rl)::numeric, 2) AS diem_rl_trung_binh
                FROM fact_ctsv WHERE diem_rl IS NOT NULL
                GROUP BY sinh_vien_key
            ),
            rl_latest AS (
                SELECT DISTINCT ON (sinh_vien_key) sinh_vien_key, xep_loai_rl, hoc_ky_key
                FROM fact_ctsv WHERE hoc_ky_key IS NOT NULL
            ...
            SELECT
                sv.sinh_vien_key, sv.ma_sinh_vien,
...
class WeeklyAggregator:
    ...
    def run(self) -> dict:
        ...
        logger.info(f"  sv_status_synced    : {sync_count:,} SV cập nhật")
    ...
    def generate(self) -> dict:
        ...
        with self.engine.connect() as conn:
            report["sv_canh_bao"] = int(conn.execute(text("""
                ...
                WHERE sv.la_ban_hien_tai = TRUE AND sv.trang_thai_hoc_tap = 'Đang học' AND agg.canh_bao_hoc_vu = TRUE
            """)).scalar() or 0)
            ...
            report["sv_hoc_bong"] = int(conn.execute(text("""
                ...
                WHERE sv.la_ban_hien_tai = TRUE AND sv.trang_thai_hoc_tap = 'Đang học'
                  AND agg.gpa_he_4 >= 3.2 AND COALESCE(agg.diem_rl_trung_binh, 0) >= 80 AND agg.co_no_hoc_phi = FALSE
            """)).scalar() or 0)

            report["ty_le_dat"] = float(conn.execute(text("""
                SELECT ROUND(100.0 * SUM(CASE WHEN dat_mon = TRUE THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0), 1)
                FROM fact_hoc_tap WHERE diem_tong_ket IS NOT NULL
            """)).scalar() or 0.0)

            rows = conn.execute(text("""
                SELECT hp.ten_mon, ROUND(100.0 * SUM(CASE WHEN f.dat_mon = FALSE THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0), 1) AS ty_le_rot
                FROM fact_hoc_tap f JOIN dim_hoc_phan hp ON f.hoc_phan_key = hp.hoc_phan_key
                WHERE f.diem_tong_ket IS NOT NULL GROUP BY hp.ten_mon HAVING COUNT(*) >= 20 ORDER BY ty_le_rot DESC LIMIT 5
            """)).fetchall()
            report["top_mon_kho"] = [{"ten_mon": r.ten_mon, "ty_le_rot": float(r.ty_le_rot or 0)} for r in rows]

            report["sv_rui_ro_cao"] = int(conn.execute(text("""
                SELECT COUNT(*) FROM agg_student_summary agg
                JOIN dim_sinh_vien sv ON agg.sinh_vien_key = sv.sinh_vien_key
                WHERE sv.la_ban_hien_tai = TRUE AND sv.trang_thai_hoc_tap = 'Đang học' AND agg.muc_do_rui_ro IN ('Cao', 'Rất cao')
            """)).scalar() or 0)
