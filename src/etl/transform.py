...
class DataTransformer: 
    ...
    def transform_all(self, extracted: ExtractedData) -> TransformedData: 
        ...
        result.dim_giang_vien = self._transform_dim_giang_vien(extracted.giang_vien, 
extracted.khoa) 
        result.dim_hoc_phan   = self._transform_dim_hoc_phan(extracted.hoc_phan, 
extracted.khoa) 
    ...
    def load_from_staging(self, run_id: str = None) -> "TransformedData": 
        ...
        data = TransformedData( 
            dim_thoi_gian  = client.download_df("dim_hoc_ky.parquet",     run_id, 
bucket="staging"), 
            dim_giang_vien = client.download_df("dim_giang_vien.parquet", run_id, 
bucket="staging"), 
            dim_hoc_phan   = client.download_df("dim_hoc_phan.parquet",   run_id, 
bucket="staging"), 
            dim_sinh_vien  = client.download_df("dim_sinh_vien.parquet",  run_id, 
bucket="staging"), 
            fact_diem      = client.download_df("fact_diem.parquet",      run_id, bucket="staging"), 
            fact_ren_luyen = client.download_df("fact_ren_luyen.parquet", run_id, 
bucket="staging"), 
            fact_tai_chinh = client.download_df("fact_tai_chinh.parquet", run_id, 
bucket="staging"), 
        ) 
    ...
    def _transform_dim_hoc_ky(self, hk_df: pd.DataFrame) -> pd.DataFrame: 
        ...
        result = hk_df[["ma_hoc_ky", "nam_hoc", "hoc_ky", "ngay_bat_dau", 
"ngay_ket_thuc"]].copy() 
    ...
    def _transform_dim_hoc_phan( 
        ...
        result["loai_hoc_phan"] = result.get("bat_buoc", pd.Series(True, 
index=result.index)).apply( 
    ...
    def _transform_fact_diem( 
        ...
        result = diem_df.merge( 
            dk_df[["ma_dang_ky", "ma_sinh_vien", "ma_hoc_phan", "ma_hoc_ky", 
"ma_giang_vien"]], 
...
errors="coerce").fillna(0) 
    ...
    def _transform_fact_tai_chinh(self, api_df: pd.DataFrame) -> pd.DataFrame: 
        ...
        if "ngay_dong_cuoi" in result.columns: 
            result["ngay_dong_cuoi"] = pd.to_datetime(result["ngay_dong_cuoi"], 
errors="coerce") 
